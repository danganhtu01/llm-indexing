...
// ── GET /export/corpus (unregistered) ────────────────────────────────────────
//
// The route was pulled from `router()` before ever going through a W13
// council review: it shipped ungated even with a submit token configured,
// read the whole (20.9 GB) vacuumed corpus into memory instead of streaming
// it, and could starve a concurrent writer past its 30s BUSY_TIMEOUT. See the
// long comment at the `/export/corpus` line in `src/service.rs`'s `router()`
// for the full finding and the four-point re-registration checklist.
//
// The behavioral tests that used to live here (404-before-publish, traversal
// / reserved-name rejection, unreadable-database reporting, a full
// vacuum-and-download round trip, and scratch-file cleanup) all depended on
// the route actually being served, so they no longer apply as written. The
// confinement logic they partly covered (`resolve_output`'s traversal and
// `jobs.sqlite` reservation checks) is exercised directly, without going
// through any route, by `valid_output_name_rejects_paths_and_wrong_extension`
// and `valid_output_name_reserves_jobs_sqlite_for_the_job_store` in
// `src/service.rs`'s own `#[cfg(test)] mod tests` — that coverage did not
// depend on this route and is unaffected by unregistering it.
//
// What's left to assert here, now that the route isn't served, is only that
// it isn't served: a plain axum "no route matched" 404, not the handler's own
// `{"error":"no corpus to export..."}` 404 body it used to return when a
// corpus hadn't been published yet.
#[tokio::test]
async fn export_corpus_route_is_not_served() {
    let temp = tempfile::tempdir().unwrap();
    let input = temp.path().join("input");
    let output = temp.path().join("output");
    let app = guard_router(&output, &input);
    ...
    // The handler's own 404 (no corpus published yet) always carried a JSON
    // `{"error": "..."}` body naming "no corpus"; axum's unmatched-route 404
    // does not, which is how this test tells "not served" apart from "served
    // and reporting absent" even with a published corpus fixture in place.
    let body = to_bytes(response.into_body(), 1024 * 1024).await.unwrap();
    assert!(
        serde_json::from_slice::<Value>(&body)
            .ok()
            .and_then(|value| value["error"].as_str().map(str::to_string))
            .is_none_or(|error| !error.contains("no corpus")),
        "expected axum's router-level 404, not the handler's own \"no corpus\" 404 \
         (i.e. the route is still being served) — body: {body:?}"
    );
}
